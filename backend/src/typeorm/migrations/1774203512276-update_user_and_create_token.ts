...
export class UpdateUserAndCreateToken1774203512276 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TABLE \`token\` (\`id\` varchar(36) NOT NULL, \`accessToken\` varchar(255) NOT NULL, \`refreshToken\` varchar(255) NOT NULL, \`accessTokenExpiresAt\` datetime NOT NULL, \`refreshTokenExpiresAt\` datetime NOT NULL, \`isBlocked\` tinyint NOT NULL DEFAULT 0, \`jti\` varchar(255) NOT NULL, \`userId\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`city\` CHANGE \`regionId\` \`regionId\` int NOT NULL`,
        );
        await queryRunner.query(
            `ALTER TABLE \`city\` CHANGE \`regionId\` \`regionId\` int NOT NULL`,
        );
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`city\` CHANGE \`regionId\` \`regionId\` int NULL`,
        );
        await queryRunner.query(
            `ALTER TABLE \`city\` CHANGE \`regionId\` \`regionId\` int NULL`,
        );
