...
const Decision: FC<PropsType> = ({id}) => {
    ...
    const handleAccept: MouseEventHandler<SVGSVGElement> = async () => {
        const response = await protectedFoodAndDrinkService.setStatus(id, FoodAndDrinkStatusEnum.ACTIVE);
        if(!response.success){
    ...
    const handleReject: MouseEventHandler<SVGSVGElement> = async () => {
        const response = await protectedFoodAndDrinkService.setStatus(id, FoodAndDrinkStatusEnum.INACTIVE)
        if(!response.success){
            setErrorMessage(response.data.message)
        }
        else{
