...
const RecoveryPasswordComponent = ({token}: PropsType) => {
    const {register, handleSubmit, errors, isShownPassword, setIsShownPassword, isShownRepeatedPassword, setIsShownRepeatedPassword, errorMessage, isValid, onSubmit} = useRecoveryPassword({token})
    return (
        <section className="h-full flex justify-center items-center">
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-[400px] p-4 recovery-password-form gap-7 items-center">
                ...
                <div className="relative mt-[5px] w-full">
                    <input type={isShownRepeatedPassword ? 'text' : 'password'} placeholder='Повторіть пароль' className='w-full pr-10 psw' {...register('repeatedPassword')}></input>
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center cursor-pointer hover:opacity-70 transition-opacity">
                        <Image src={Vision} alt={"Показати пароль"} height={25} width={25} onClick={() => {
                ...
                <button type="submit" className="recovery-password-btn bg-black text-white mt-10" disabled={!isValid}
