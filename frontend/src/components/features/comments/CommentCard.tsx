...
const CommentCard: FC<PropsType> = ({comment, user, isOwner, foodAndDrinkId, ref}) => {
    ...
    return (
        <Card ref={ref} className="flex-row justify-between">
            ...
            {user && (user.role === GlobalUserRoleEnum.SUPERADMIN || user.id === creatorId || isOwner) && <FoodAndDrinkCommentCardDropdown commentId={id} foodAndDrinkId={foodAndDrinkId} setIsEdited={setIsEdited} isOwner={isOwner} user={user} creator={comment.user} text={text}/>}
        </Card>
    )
}
