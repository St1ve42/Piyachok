import {ChangeEvent, ChangeEventHandler, KeyboardEventHandler, useEffect, useMemo, useRef, useState} from "react";
...
const useFoodAndDrinkCreatingOrUpdatingView = ({mode, foodAndDrink, urlToRedirect}: PropsType) => {
    ...
    useEffect(() => {
        if(mode === 'update' && foodAndDrink){
          const {images, businessHours, features, tags, city, region, type, description} = foodAndDrink
          if(tags){
            setTags(tags)
          }
          if(features){
          ...
          if(images){
            Promise.all(images.map(async image => await utilsService.urlToFile(utilsService.buildStorageURL(image), image))).then(images => setGalleryFiles(images))
          }
        ...
        if(galleryFiles.length < 10){
            ...
            if (file) {
                ...
                if(!allowedExtensions.includes(extension)){
                    setErrorMessage(`Недопустиме розширення файлу. Дозволені розширення: ${utilsService.outputArray(allowedExtensions)}.`)
                    return;
                }
                if(file.size > 1024*1024){
                    setErrorMessage('Розмір файла не може перевищувати 1 МБ.')
                    return;
                }
    ...
    const handleCreateFormSubmit = async (data: ICreateFoodAndDrink) => {
        ...
        let coordinatesResponse = await utilsService.getCoordinates({
            region: regionInputValue,
            city: cityInputValue,
            street
        })
        if(!coordinatesResponse.success){
            coordinatesResponse = await utilsService.getCoordinates({
                region: regionInputValue,
                city: cityInputValue,
            })
            if(!coordinatesResponse.success){
