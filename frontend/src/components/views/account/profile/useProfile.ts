...
const useProfile = ({user, type, id}: {user: IUser, type: 'user' | 'superadmin', id?: string}) => {
    const {city, region, isActive, isVerified, email} = user
    ...
    const {register, handleSubmit, control, formState: {errors, isValid}, reset} = useForm<IUpdateMe>({
