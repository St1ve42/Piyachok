...
const SignUpComponent = () => {
    const {formState, formAction, isOpenRegion, setIsOpenRegion, isShownPassword, setIsShownPassword, isOpenCity, setIsOpenCity, regionId, regionInputValue, cityInputValue, errors, register, isValid, regionQuery, cityQuery, ref, handleRegionInputChange, handleCityInputChange, handleRegionSelect, handleCitySelect} = useSignUp()
    ...
    return (
        <section className="h-full flex justify-center items-center ">
            <form action={formAction} className="flex flex-col w-[50%] p-4 sign-up-form gap-5">
                ...
                {errors.name && <div className="text-red-600 text-xs leading-none">{errors.name.message}</div>}
                {/*</div>*/}
                {errors.surname && <div className="text-red-600 text-xs leading-none">{errors.surname.message}</div>}
                <input type='number' placeholder='Вік' min={1} {...register('age')}></input>
                {errors.age && <div className="text-red-600 text-xs leading-none">{errors.age.message}</div>}
                ...
                {errors.regionId && <div className="text-red-600 text-xs leading-none">{errors.regionId.message}</div>}
                {errors.cityId && <div className="text-red-600 text-xs leading-none">{errors.cityId.message}</div>}
                ...
                {errors.email && <div className="text-red-600 text-xs leading-none">{errors.email.message}</div>}
                ...
                {/*fix issue about submit without region and city after retrieving formData*/}
                {errors.password && <div className="text-red-600 text-xs leading-none">{errors.password.message}</div>}
                {formState.data.message !== '' && !formState.success && <div className="text-red-600 text-xs leading-none">{formState.data.message}</div>}
                ...
                <div className="text-right">Маєте акаунт? <Link href={'/auth/sign-in'} className="text-blue-600">Увійти</Link></div>
            </form>
        </section>
    )
}
